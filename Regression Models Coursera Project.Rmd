---
title: "Regression Models Coursera  - Motor Trend Data Analysis Report"
...
attach(mtcars)
```   
Exploratory data analysis. In the **Appendix: Figures** section the corresponding plots are shown. According to the box plot, we see that manual transmission yields higher values of MPG in general. And as for the pair graph, we can see some higher correlations between variables like "wt", "disp", "cyl" and "hp".  
...
We use backward selection to select some statistically significant variables.  
```{r, results='hide'}
...
Let us fit the simple model with MPG as the outcome variable and Transmission as the predictor variable.  
```{r, results='hide'}
